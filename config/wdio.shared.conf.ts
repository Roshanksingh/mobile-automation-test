...
export const config: WebdriverIO.Config = {
    ...
    reporters: ['spec',
        ['allure', {
            outputDir: './report/allure/allure-results',
    ...
    //     (global as any).allure = allure;
    //     allure.addFeature(suite.name);
    //     allure.addDescription("generating Allure reports " + (suite as any).name);
    // },
    beforeTest: function (test, context) {
        allure.addEnvironment("DEVICE", (driver.capabilities as any).platform);
        allure.addEnvironment("DEVICE_VERSION", (driver.capabilities as any).platformVersion);
        allure.addTestId("TC-001" + test.fullName);
    },

    beforeSuite: function (suite) {
        allure.addFeature(suite.title);
    },
